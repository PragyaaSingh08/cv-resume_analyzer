...
from function import file_loader, load_vector_store
...
def getquery():
    ...
    queryfile_path = os.path.join(upload_folder, file.filename)
...
def analyze_resume():
    try:
        ...
        if  not query_file:
            return jsonify({'error': 'query are required.'}), 400
        # if file_name not in retriever_storage:
            # return jsonify({'error': 'File not found or not processed yet.'}), 404
